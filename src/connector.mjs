...
export async function getWalletConnector(
  ...
  if (typeof requestedNetwork === "string") {
    if (requestedNetwork in networks) {
      ...
      throw new Error(
        `Invalid or unknown Seacrest network: ${requestedNetwork}`,
      );
    }
  }
      ...
      signClient.connect({
        ...
        approval().then((session) => {
          ...
          if (matchingChainAccounts.length === 0) {
            console.info(
              `[Seacrest][WalletConnect] Error: connected to Seacrest with accounts ${
                JSON.stringify(chainAccounts)
              }, but requested network "${requestedNetwork}". Please change networks and try again.`,
            );
            ...
            console.info(
              `[Seacrest][WalletConnect] Connected to Seacrest with accounts ${
                JSON.stringify(
                  accounts,
                )
              }`,
        ...
        if (uri) {
          ...
          console.error(
            `[Seacrest][WalletConnect] Error: No uri returned: uri=${uri}, approval=${approval}`,
          );
        }
      });
    });
